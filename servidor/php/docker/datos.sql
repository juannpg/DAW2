...
ALTER TABLE `producto` ADD CONSTRAINT `producto_ibfk_1` FOREIGN KEY (`familia`) REFERENCES `familia` (`id`) ON UPDATE CASCADE;

ALTER TABLE `stock` ADD CONSTRAINT `stock_ibfk_2` FOREIGN KEY (`tienda`) REFERENCES `tienda` (`id`) ON UPDATE CASCADE,
ADD CONSTRAINT `stock_ibfk_1` FOREIGN KEY (`producto`) REFERENCES `producto` (`id`) ON UPDATE CASCADE;
...
VALUES
  (
    ...
    'Nintendo 3DS negro',
    'Nintendo',
    'Consola portátil de Nintendo que permitirá disfrutar de efectos 3D sin necesidad de gafas especiales, e incluirá retrocompatibilidad con el software de DS y de DSi.',
    '270.00',
    2
  ),
  (
  ...
  (
    ...
    'Canon A3100',
    'La cámara PowerShot A3100 IS, inteligente y compacta, presenta la calidad de imagen de Canon en un cuerpo\r\ncompacto y ligero para capturar fotografías sin esfuerzo; es tan fácil como apuntar y disparar.\r\nCaracterísticas:\r\n12,1 MP \r\nZoom óptico 4x con IS \r\nPantalla LCD de 6,7 cm (2,7") ',
    '101.40',
    1
  ),
  (
...
INSERT INTO
  `stock` (`producto`, `tienda`, `unidades`)
VALUES
  (1, 1, 2),
  (1, 2, 1),
  (2, 1, 1),
